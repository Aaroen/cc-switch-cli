...
if [ ! -d "src-tauri/target/release" ]; then
    ...
    if [ -n "$(find src-tauri/src -name '*.rs' -newer src-tauri/target/release 2>/dev/null)" ]; then
        NEED_REBUILD=true
    elif [ -f "src-tauri/Cargo.toml" ] && [ "src-tauri/Cargo.toml" -nt "src-tauri/target/release" ]; then
        NEED_REBUILD=true
    fi
fi

if [ "$NEED_REBUILD" = true ]; then
    # 运行Tauri构建，即使打包失败也继续（只要二进制文件成功编译）
    pnpm tauri build > "$LOG_DIR/tauri_build.log" 2>&1 || true
...
# 默认为所有应用启用权重轮询
if [ "$CC_DISABLE_WEIGHT_ROUND_ROBIN" != "true" ]; then
    # 使用 CLI 命令启用权重轮询
    WEIGHT_RR_ENABLED=0
    ...
    step_done $CURRENT_STEP $TOTAL_STEPS "权重轮询 (已禁用)"
fi
...
if [ "$PROXY_OK" = true ]; then
    ...
    echo -e "   日志: tail -f $LOG_DIR/server.log"
else
